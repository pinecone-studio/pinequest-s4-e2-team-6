        ...
        <div className="absolute left-1/2 top-[46%] w-[92%] max-w-sm -translate-x-1/2 -translate-y-1/2">
          <div className="glass-panel rounded-[28px] p-6">
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                <h2 className="text-3xl font-black tracking-[0]">Мэлхий хад</h2>
                <p className="mt-1 text-black/60 dark:text-white/60">Тэрэлж, Монгол</p>
              </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ["headphones", "Сонсох"],
                ["translate", "Орчуулах"],
                ["bookmark_add", "Хадгалах"],
              ].map(([icon, label]) => (
                <button key={label} type="button" className="flex flex-col items-center gap-2 text-[#00658b] transition hover:scale-105 dark:text-[#7dd0ff]">
                  <span className="grid size-12 place-items-center rounded-full bg-[#00658b]/10">
                    <MaterialIcon name={icon} />
                  </span>
                  <span className="text-[10px] font-black uppercase tracking-[0]">{label}</span>
                </button>
              ))}
            </div>
          </div>
